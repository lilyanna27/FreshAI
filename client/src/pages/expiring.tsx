...
export default function Expiring() {
  ...
  return (
    <div className="p-4">
      <div className="mb-6">
        <h2 className="text-xl font-semibold text-rose-ebony mb-2">About to Expire</h2>
        <p className="text-gray-500 text-sm">Items that need your attention</p>
      </div>

      {totalExpiringItems === 0 ? (
        <div className="bg-white rounded-xl p-8 shadow-sm border border-gray-100 text-center">
          <CheckCircle className="mx-auto text-apple-green mb-3" size={48} />
          <p className="text-gray-500 text-lg mb-2">All items are fresh!</p>
          <p className="text-gray-400">No items are expiring soon</p>
        </div>
      ) : (
        <>
          {/* Expiring Today */}
          {categorizedItems.today.length > 0 && (
            <div className="mb-6">
              ...
              {categorizedItems.today.map((item) => {
                ...
                return (
                  <div key={item.id} className="bg-white rounded-xl p-4 shadow-sm border border-red-200 mb-3">
                    <div className="flex items-center">
                      <div className="w-12 h-12 bg-red-100 rounded-xl flex items-center justify-center mr-4">
                        <AlertTriangle className="text-vermillion" size={20} />
                      </div>
                      <div className="flex-1">
                        <h4 className="font-medium text-rose-ebony">{item.name}</h4>
          ...
          {categorizedItems.soon.length > 0 && (
            <div className="mb-6">
              ...
              {categorizedItems.soon.map((item) => {
                const expirationInfo = getExpirationStatus(new Date(item.expirationDate));
                return (
                  <div key={item.id} className="bg-white rounded-xl p-4 shadow-sm border border-yellow-200 mb-3">
                    <div className="flex items-center">
          ...
          {categorizedItems.expired.length > 0 && (
            <div>
              ...
              {categorizedItems.expired.map((item) => {
                const expirationInfo = getExpirationStatus(new Date(item.expirationDate));
                return (
                  <div key={item.id} className="bg-white rounded-xl p-4 shadow-sm border border-gray-200 mb-3 opacity-75">
                    <div className="flex items-center">
                      ...
                      <div className="flex-1">
                        <h4 className="font-medium text-rose-ebony">{item.name}</h4>
                        <p className="text-sm text-gray-500">{item.quantity}</p>
                        <p className="text-xs text-gray-500 font-medium">{expirationInfo.message}</p>
                      </div>
                      <Button
