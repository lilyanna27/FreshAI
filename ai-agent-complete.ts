...
class EnhancedMemorySystem {
  ...
  async extractAndSavePreferences(
    ...
    for (const pattern of dislikePatterns) {
      ...
      while ((match = pattern.exec(lowerText)) !== null) {
        ...
        if (
          !existingMemories.memories?.some((m) => m.content.includes(disliked))
        ) {
    ...
    for (const pattern of likePatterns) {
      ...
      while ((match = pattern.exec(lowerText)) !== null) {
        ...
        if (
          !existingMemories.memories?.some((m) => m.content.includes(liked))
        ) {
          await this.saveMemory(
            userId,
            `User likes ${liked}`,
    ...
    for (const cuisine of cuisineTypes) {
      if (lowerText.includes(cuisine)) {
        ...
        if (
          !existingMemories.memories?.some((m) => m.content.includes(cuisine))
        ) {
          await this.saveMemory(
            userId,
            `User prefers ${cuisine} cuisine`,
            namespace,
            "semantic",
          );
    ...
    for (const dietary of dietaryTerms) {
      if (
        lowerText.includes(dietary) ||
        lowerText.includes(dietary.replace("-", " "))
      ) {
        const existingMemories = await this.searchMemories(
          userId,
          `dietary ${dietary}`,
        ...
        if (
          !existingMemories.memories?.some((m) => m.content.includes(dietary))
        ) {
          await this.saveMemory(
            userId,
            `User has ${dietary} dietary restriction`,
            namespace,
            "semantic",
          );
  ...
  async saveMemory(
    ...
    try {
      ...
      return {
        status: "error",
        message: `Failed to save ${type} memory: ${error}`,
      };
    }
  }
  ...
  async saveEpisode(
    userId: string,
    conversation: ConversationEpisode,
    namespace: string = "agent_memories/{user_id}",
  ): Promise<MemoryResult> {
  ...
  async searchMemories(
    ...
    query: string,
    namespace: string,
    type: "semantic" | "episodic" | "all" = "all",
    limit: number = 5,
  ): Promise<MemoryResult> {
    try {
  ...
  applyProceduralMemoryRules(
    userQuery: string,
    userProfile: UserProfile,
    pastEpisodes: MemoryEntry[],
  ): {
    ...
    if (userProfile.dietary.length > 0) {
      result.recommendations.push(
        `Include ${userProfile.dietary.join(" and ")} substitutions in recipes`,
      );
    }
    ...
    const feedbackEpisodes = pastEpisodes.filter(
      (ep) =>
        ep.content.includes("like") ||
        ep.content.includes("love") ||
        ep.content.includes("hate") ||
        ep.content.includes("dislike"),
    );
    if (feedbackEpisodes.length > 0) {
      result.recommendations.push(
        "Consider past feedback when suggesting recipes",
      );
    }
  ...
  async getConversationContext(
    ...
    const episodeResult = await this.searchMemories(
      userId,
      currentQuery,
      namespaceResolved,
      "episodic",
    ...
    const semanticResult = await this.searchMemories(
      userId,
      currentQuery,
      namespaceResolved,
      "semantic",
    ...
    const proceduralGuidance = this.applyProceduralMemoryRules(
      currentQuery,
      userProfile,
      relevantEpisodes,
    );
...
const getSystemPrompt = (
  userProfile: any,
  contextInfo: any,
  recipesGenerated: boolean = false,
) => {
...
export class ConversationalAgent {
  ...
  async processConversation(
    ...
    const conversationContext = await this.memorySystem.getConversationContext(
      userId,
      userMessage,
    );
    const {
    ...
    const learningResult = await this.memorySystem.extractAndSavePreferences(
      userId,
      userMessage,
    );
    ...
    if (messages.length === 0) {
      ...
      const updatedSystemPrompt = getSystemPrompt(
        userProfile,
        contextInfo,
        isRecipeRequest,
      );
    ...
    const reasoning = [
      {
        ...
        action: isRecipeRequest
          ? "Preparing personalized recipe generation"
          : "Providing contextual cooking assistance",
    ...
    if (isRecipeRequest) {
      ...
      let availableIngredients = fridgeIngredients.filter(
        (ingredient) =>
      ...
      availableIngredients.forEach((ingredient) => {
        ...
        if (substitutes.length > 0) {
          enhancedIngredients.push(
            `${ingredient} (or substitute: ${substitutes.slice(0, 2).join(", ")})`,
          );
        }
      });

      // Extract parameters from message
      const peopleMatch = userMessage.match(
        /(\d+)\s*(people|person|servings?)/i,
      );
      ...
      recipes = await this.generateSampleRecipes(
        enhancedIngredients,
        userProfile,
        num_people,
      );
    ...
    if (isRecipeRequest && recipes.length > 0) {
      const recipeNote = new HumanMessage(
        `[SYSTEM NOTE: ${recipes.length} personalized recipes have been generated based on your preferences and will be displayed in recipe cards. Please acknowledge this without repeating the recipe details.]`,
      );
    ...
    if (allNewLearnings.length > 0) {
      await this.memorySystem.saveTakeaway(
        userId,
        `User expressed new preferences: ${allNewLearnings.join(", ")}`,
      );
    }
  ...
  private async generateSampleRecipes(
    ingredients: string[],
    userProfile: UserProfile,
    servings: number,
